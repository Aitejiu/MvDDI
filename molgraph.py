...
class MolCollator(object):
    ...
    def __call__(self, batch):
        ...
        negtypes_batch = []
        rels_batch = []
        for posdataa, posdatab, negdata, negtype, rels in batch:
            possmilesa_batch.append(posdataa.smiles)
            possmilesb_batch.append(posdatab.smiles)
            negsmiles_batch.append(negdata.smiles)
            posfeaturesa_batch.append(posdataa.features)
            posfeaturesb_batch.append(posdatab.features)
            negfeatures_batch.append(negdata.features)
            negtypes_batch.append(negtype)
            rels_batch.append(rels)
        posdataa_batch_mol_graph = mol2graph(possmilesa_batch, self.shared_dict, self.args)
        posdatab_batch_mol_graph = mol2graph(possmilesb_batch, self.shared_dict, self.args)
        negdata_batch_mol_graph = mol2graph(negsmiles_batch, self.shared_dict, self.args)
        batcha = posdataa_batch_mol_graph.get_components()
        batchb = posdatab_batch_mol_graph.get_components()
        negbatch = negdata_batch_mol_graph.get_components()
        rels_batch = torch.tensor(rels_batch)
        return possmilesa_batch, possmilesb_batch, negsmiles_batch, batcha, batchb, negbatch, posfeaturesa_batch, posfeaturesb_batch, negfeatures_batch, negtypes_batch, rels_batch