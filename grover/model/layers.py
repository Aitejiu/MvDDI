...
class Readout(nn.Module):
    ...
    def __init__(self,
        ...
        # Cached zeros
        self.cached_zero_vector_atom = nn.Parameter(torch.zeros(hidden_size).repeat(87,1), requires_grad=False)
        self.cached_zero_vector_bond = nn.Parameter(torch.zeros(hidden_size).repeat(184,1), requires_grad=False)
        self.rtype = "mean"
    ...
    def forward(self, embeddings, scope, type=None):
        """
        ...
        for _, (a_start, a_size) in enumerate(scope):
            if a_size == 0:
                if type == 'bond':
                    mol_vecs.append(self.cached_zero_vector_bond)
                else:
                    mol_vecs.append(self.cached_zero_vector_atom)
            else:
                cur_hiddens = embeddings.narrow(0, a_start, a_size)
                # print('cur_hiddens',cur_hiddens.shape)
                if self.rtype == "self_attention":
                    g_hiddens, attn = self.attn(cur_hiddens)
                    g_hiddens = g_hiddens.flatten()
                    # Temporarily disable. Enable it if you want to save attentions.
                    # self.attns.append(attn.cpu().detach().numpy())
                else:
                    g_hiddens = cur_hiddens.sum(dim=0) / a_size
                if type == 'bond':
                    if len(cur_hiddens) > 184:
                        cur_hiddens = cur_hiddens[:184]
                    else:
                        cur_hiddens = torch.cat((cur_hiddens,g_hiddens.repeat(184-len(cur_hiddens),1)),0)
                else:
                    if len(cur_hiddens) > 87:
                        cur_hiddens = cur_hiddens[:87]
                    else:
                        cur_hiddens = torch.cat((cur_hiddens,g_hiddens.repeat(87-len(cur_hiddens),1)),0)
