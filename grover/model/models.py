...
class GroverFpGeneration(nn.Module):
    ...
    def forward(self, batch, features_batch):
        ...
        # print('a_scope',a_scope.shape)
        # print('b_scope',b_scope.shape)
        output = self.grover(batch)
        # return output
        # Share readout
        mol_atom_from_bond_output = self.readout(output["atom_from_bond"], a_scope, 'atom')
        mol_atom_from_atom_output = self.readout(output["atom_from_atom"], a_scope, 'atom')
        # mol_atom_from_bond_output = output["atom_from_bond"]
        # mol_atom_from_atom_output = output["atom_from_atom"]
        # if self.fingerprint_source == "bond" or self.fingerprint_source == "both":
        mol_bond_from_atom_output = self.readout(output["bond_from_atom"], b_scope, 'bond')
        mol_bond_from_bodd_output = self.readout(output["bond_from_bond"], b_scope, 'bond')
        # mol_bond_from_atom_output = output["bond_from_atom"]
        # mol_bond_from_bodd_output = output["bond_from_bond"]
        # print('mol_atom_from_bond_output',mol_atom_from_bond_output.shape)
        # print('mol_atom_from_atom_output',mol_atom_from_atom_output.shape)
        # print('mol_bond_from_atom_output',mol_bond_from_atom_output.shape)
        # print('mol_bond_from_bodd_output',mol_bond_from_bodd_output.shape)
        ...
        #     fp = torch.cat([fp, features_batch], 2)
        # print('fp.shape',fp.shape)
        return torch.cat([mol_atom_from_atom_output, mol_atom_from_bond_output], 2), torch.cat([mol_bond_from_atom_output, mol_bond_from_bodd_output], 2), features_batch
